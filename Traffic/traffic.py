...
def load_data(data_dir):
    ...
    for folder in os.listdir(data_dir):
        ...
        if os.path.isdir(directory):
            ...
            for pic_file in os.listdir(directory):
                image = cv2.imread(os.path.join(directory, pic_file), cv2.IMREAD_COLOR)
                image = cv2.resize(image, (IMG_WIDTH, IMG_HEIGHT), interpolation=cv2.INTER_AREA)
                images.append(image)
                labels.append(int(folder))
    
    picture = (images,labels)
    return picture
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        # Add an output layer with output units for all catgories
